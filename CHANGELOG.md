...
## v4.3.6 (2026-08-29)
- v4.3.6: 工具箱卡片懒加载（首屏 12 个 API 请求降到 2 个，折叠分组点开才加载）

## v4.3.5 (2026-08-29)
- v4.3.5: 工具箱卡片懒加载（首屏 12 个 API 请求降到 2 个）

## v4.3.4 (2026-08-29)
- v4.3.4 审核修复3: 根目录输入拦截 + 受保护路径拒绝入列（避免加进去却不清理的误导）

## v4.3.3 (2026-08-29)
- v4.3.3 审核修复2: 根目录输入拦截 + 受保护路径拒绝入列（避免加进去却不清理的误导）

## v4.3.2 (2026-08-29)
- v4.3.2 审核修复: 危险路径守卫（拒绝存储根/系统分区/模块数据目录）+ 受保护路径拒绝入列 + 定时循环启动即计时 + 移除 SDROOT 硬编码复用 sdcardRoots

## v4.3.1 (2026-08-29)
- v4.3.0: WebUI 4tab + 定时清理页 + 保护路径 + 规则库 + 工具箱三分组 + 设置页折叠（对齐 APP v3.0.28）

## v4.2.22 (2026-08-27)
- 修复: SHA256校验失败时只删下载的临时文件,不误删用户指定路径

## v4.2.21 (2026-08-27)
- P0安全: CORS白名单+热更新SHA256校验+原子写入+日志轮转+优雅退出

## v4.2.20 (2026-08-27)
- 子功能6项: 长按加白名单+APK已装检测+回收站剩余天数+应用批量清理+更新日志预览+更新红点

## v4.2.19 (2026-08-27)
- UI全面优化: 无障碍(减弱动效/聚焦环)+OLED真省电+Toast四类型底部+切Tab回顶+今日徽章+关于页+长按确认

## v4.2.18 (2026-08-27)
- 深挖: AI一键采纳+大文件类型Tab+扫描根目录可配置

## v4.2.17 (2026-08-27)
- 定时任务支持动作选择: 清理/整理/两者/默认两者

## v4.2.16 (2026-08-27)
- v4.3.0: 主题切换(深色/OLED/浅色) + 强调色四选一 + 配置持久化

## v4.2.15 (2026-08-27)
- 热更新一键自动: 内置update.json拉最新,用户无需填链接
